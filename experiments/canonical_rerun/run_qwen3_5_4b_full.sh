...
for SEED in "${SEEDS[@]}"; do
  ...
  bash $SCRIPT_DIR/C1_sp_mlp_pretrain_ca.sh "$MODEL" "$SEED" "$GPU" 2>&1 | tail -20 | tee -a "$DRIVER_LOG"
  echo "[driver] === C3 T-LSTM stage1 seed=$SEED ===" | tee -a "$DRIVER_LOG"
  bash $SCRIPT_DIR/C3_t_lstm_pretrain_ca.sh "$MODEL" "$SEED" "$GPU" 2>&1 | tail -20 | tee -a "$DRIVER_LOG"
done

# Stage 2: C2, C4, C5 in parallel-by-seed (sequential per seed).
for SEED in "${SEEDS[@]}"; do
  echo "[driver] === C2 SP-MLP stage2 seed=$SEED ===" | tee -a "$DRIVER_LOG"
  bash $SCRIPT_DIR/C2_sp_mlp_mn_head.sh "$MODEL" "$SEED" "$GPU" 2>&1 | tail -15 | tee -a "$DRIVER_LOG"
  echo "[driver] === C4 T-LSTM stage2 seed=$SEED ===" | tee -a "$DRIVER_LOG"
  bash $SCRIPT_DIR/C4_t_lstm_mn_head.sh "$MODEL" "$SEED" "$GPU" 2>&1 | tail -15 | tee -a "$DRIVER_LOG"
  echo "[driver] === C5 TME v3-B e2e from scratch seed=$SEED ===" | tee -a "$DRIVER_LOG"
  bash $SCRIPT_DIR/C5_tme_e2e_mn.sh "$MODEL" "$SEED" "$GPU" 2>&1 | tail -15 | tee -a "$DRIVER_LOG"
done
