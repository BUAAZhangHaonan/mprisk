...
class TrainingBudget:
    ...
    train_fraction_seed: int = 20260717
...
def load_probe_config(path: str | Path) -> ProbeConfig:
    ...
    train_fraction = float(raw_training["train_fraction"])
    if not 0.0 < train_fraction <= 1.0:
        raise ValueError("training.train_fraction must be in (0.0, 1.0]")
    training = TrainingBudget(
        ...
        train_fraction_seed=int(raw_training["train_fraction_seed"]),
    )
...
def _run_representation(
    *,
    ...
    train_samples = _stratified_fraction(
        train_samples,
        fraction=config.training.train_fraction,
        seed=config.training.train_fraction_seed,
    )
...
def _stratified_fraction(
    samples: list[ProbeSample], *, fraction: float, seed: int
) -> list[ProbeSample]:
    """Stratified subsample by misread label, deterministic per (fraction, seed)."""
    if fraction >= 1.0:
        return samples
    by_label: dict[int, list[ProbeSample]] = {0: [], 1: []}
    for s in samples:
        by_label[s.label_id].append(s)
    rng = random.Random(seed)
    picked: list[ProbeSample] = []
    for label_id in (0, 1):
        rows = sorted(by_label[label_id], key=lambda s: s.sample_id)
        keep = max(1, math.ceil(len(rows) * fraction)) if rows else 0
        rng.shuffle(rows)
        kept = sorted(rows[:keep], key=lambda s: s.sample_id)
        picked.extend(kept)
    if len({s.label_id for s in picked}) != 2:
        raise ValueError("train_fraction removed a Misread class")
    return picked
