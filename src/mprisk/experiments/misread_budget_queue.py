...
def derive_ready_probe_config(
    ...
    return {
        ...
        "training": {
            **dict(plan.training),
            "train_fraction": source.fraction,
            "train_fraction_seed": plan.training.get("seed", 20260717),
        },
...
def _audit_probe_run(
    ...
    # Compare training budget ignoring per-fraction fields (train_fraction and
    # train_fraction_seed are derived from the source fraction / seed, not the plan).
    marker_budget = {
        k: v
        for k, v in marker["training_budget"].items()
        if k not in {"train_fraction", "train_fraction_seed"}
    }
    if marker_budget != plan.training:
