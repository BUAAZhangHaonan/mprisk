...
class Gemma4Wrapper(BaseModelWrapper):
    ...
    def extract_prefill(self, request: PrefillRequest) -> PrefillResult:
        ...
        provenance = {
            ...
            "media_keys": sorted(k for k, v in media.items() if v is not None),
        }
    ...
    def _validate_request(self, request: PrefillRequest) -> None:
        ...
        if request.protocol != "va":
            raise ValueError(
                "Gemma-4 wrapper currently supports protocol VA only; "
...
def _collect_media_inputs(request: PrefillRequest) -> dict[str, Any]:
    ...
    for message in request.messages:
        for item in message.get("content", []):
            ...
            if ctype == "audio":
                audio_paths.append(_audio_to_wav(str(item.get("audio"))))
            elif ctype == "video":
    ...
    media: dict[str, Any] = {"audio": None, "videos": None, "images": None, "audio_waveforms": None}
    if audio_paths:
...
def _audio_to_wav(source_path: str) -> str:
    ...
    _TEMP_FILES_TO_CLEAN.append(tmp.name)
    return tmp.name
...
def _video_to_frames_with_audio(video_path: str, *, max_frames: int = MAX_VIDEO_FRAMES) -> tuple:
    """Decode video frames AND the co-located audio track from the same mp4.

    Returns (frames_ndarray, audio_ndarray, sample_rate). Audio is mono 16kHz
    so processor accepts it as the joint audio for the video.
    """
    import av
    import numpy as np
    ...
    if not audio_chunks:
        return frames, None, sample_rate
    audio = np.concatenate(audio_chunks, axis=1)  # [channels, total_samples]
    # Convert to mono by averaging channels; ensure float32
    if audio.shape[0] > 1:
        audio = audio.mean(axis=0, keepdims=True)
    audio = audio[0].astype(np.float32) / 32768.0  # int16 -> float
