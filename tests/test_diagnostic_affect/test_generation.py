...
def _plan(tmp_path: Path, *, protocol: str = "VT"):
    return build_diagnostic_affect_description_plan(
        schema_name="mprisk_diagnostic_affect_description_config",
...
def test_plan_uses_explicit_identity_and_does_not_leak_annotations(tmp_path: Path) -> None:
    ...
    assert plan.signature["schema_name"] == (
        "mprisk_diagnostic_affect_description_signature"
    )
    assert plan.signature["run_id"] == "diagnostic-affect-test"
...
def test_ledger_and_export_use_diagnostic_affect_description_field(tmp_path: Path) -> None:
    ...
    assert row["schema_name"] == "mprisk_diagnostic_affect_description"
    assert row["run_id"] == "diagnostic-affect-test"
