...
try:
    ...
    from scripts.run_state_measurement_smoke import EmbeddingBuildResult, build_embedding_manifest
    ...
    from run_state_measurement_smoke import EmbeddingBuildResult, build_embedding_manifest
...
from mprisk.representation.adapters import RAW_REPR_KEYS, TME_SUPCON_V1_REPR_KEY
...
def _export_embeddings(
    *,
...
) -> EmbeddingBuildResult | TrainedEmbeddingExportResult:
    ...
    if repr_key == TME_SUPCON_V1_REPR_KEY:
        if checkpoint is None:
            raise ValueError(
                "repr_key 'tme_supcon_v1' requires --checkpoint; "
                "this pipeline does not train checkpoints automatically"
            )
        return export_trained_embeddings(
            bundle_manifest_path=bundle_manifest_path,
            checkpoint_path=checkpoint,
            output_dir=output_dir,
            repr_key=repr_key,
        )
    supported = ", ".join(sorted([*RAW_REPR_KEYS, TME_SUPCON_V1_REPR_KEY]))
    raise ValueError(f"Unsupported repr_key {repr_key!r}; expected one of {supported}")


def _validate_repr_request(*, repr_key: str, checkpoint: str | Path | None) -> None:
    if repr_key == TME_SUPCON_V1_REPR_KEY and checkpoint is None:
        raise ValueError(
            "repr_key 'tme_supcon_v1' requires --checkpoint; "
            "this pipeline does not train checkpoints automatically"
        )
...
def _write_core_summary(
    *,
    ...
    state_dataset_result: StateDatasetBuildResult,
    bundle_result: StateBundleBuildResult,
    embedding_result: EmbeddingBuildResult | TrainedEmbeddingExportResult,
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--thresholds",
        default=json.dumps({"kappa": 0.5, "tau": 0.25, "delta": 0.2}),
