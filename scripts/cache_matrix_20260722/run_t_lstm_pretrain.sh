#!/bin/bash
# cache_matrix_20260722: T-LSTM v2 Stage-1 (C/A pretrain).
...
LOG=outputs/cache_matrix_20260722/_logs/t_lstm_pretrain_${MODEL}_seed${SEED}.log
mkdir -p "$(dirname "$LOG")"
...
echo "[T-LSTM pretrain] MODEL=$MODEL SEED=$SEED GPU=$GPU proto=$PROTO"
...
touch "${LOG}.done"
echo "[T-LSTM pretrain] DONE -> $OUT/pretrain_metrics.json"
