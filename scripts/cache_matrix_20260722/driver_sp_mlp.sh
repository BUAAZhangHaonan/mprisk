#!/usr/bin/env bash
# Driver: SP-MLP wave (cache_matrix_20260722) — 2-stage pretrain+mn_head per cell.
...
for cell in "${CELLS[@]}"; do
  ...
  (
    bash scripts/cache_matrix_20260722/run_sp_mlp_pretrain.sh "$MODEL" "$SEED" "$free_gpu" \
      > "outputs/cache_matrix_20260722/_logs/sp_mlp_pretrain_${MODEL}_seed${SEED}.driver.log" 2>&1
    if [ -f "$OUT/encoder.pt" ] && [ ! -f "$OUT/mn_metrics.json" ]; then
      bash scripts/cache_matrix_20260722/run_sp_mlp_mn_head.sh "$MODEL" "$SEED" "$free_gpu" \
        > "outputs/cache_matrix_20260722/_logs/sp_mlp_mn_head_${MODEL}_seed${SEED}.driver.log" 2>&1
    fi
  ) &
  gpu_busy[$free_gpu]=$!
done
