#!/bin/bash
# cache_matrix_20260722: T-LSTM v2 Stage-2 (M/N head on frozen encoder).
...
# Args: MODEL SEED GPU
# Depends on: pretrain (encoder.pt in t_lstm output dir).
# Output: outputs/cache_matrix_20260722/runs/t_lstm/${MODEL}_seed${SEED}/
set -euo pipefail
...
OUT=outputs/cache_matrix_20260722/runs/t_lstm/${MODEL}_seed${SEED}
...
LOG=outputs/cache_matrix_20260722/_logs/t_lstm_mn_head_${MODEL}_seed${SEED}.log
mkdir -p "$(dirname "$LOG")"

echo "[T-LSTM mn_head] MODEL=$MODEL SEED=$SEED GPU=$GPU ckpt=$ENCODER_CKPT"
...
touch "${LOG}.done"
echo "[T-LSTM mn_head] DONE -> $OUT/mn_metrics.json"
